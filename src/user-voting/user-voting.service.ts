...
export class UserVotingService {
	...
	async validationCreate(userFromId: number, userVoting: UserVotingEntity, type: UserVotingType): Promise<TValidation> {
		...
		if (type === UserVotingType.final) {
			votesHave = await this.leftVotesCommissionFinal(
				nominationOrder.nominationId,
				userVoting.userFromId,
				userVoting.nominationOrderId,
				'final',
			);
		}
	...
	async leftVotesCommissionFinal(nominationId, userFromId, nominationOrderId, type = 'сommission'): Promise<{ votes: number[], error: string }> {
		try {
			...
			const haveVoteIsNominationOrder = await this.findAll({ userFromId, type }, undefined, undefined);
	...
	async leftVotes(nominationId, userFromId, nominationOrderId): Promise<{ votes: number[], error: string }> {
		try {
			...
			const allNominationOrderByNomination = await this.nominationOrderService.findAllPublic({
				nominationId,
				stateId: userFrom.stateId,
			});
	...
	async calculateCommission(nomination: number, type = 'сommission'): Promise<ResultUserVotingDto[]> {
		try {
			// Поулчаем сгруппированный список из базы данных по nominationId
			const groupByNominationID: TGroupByNominationID[] = await this.userVotingRepository.sequelize.query(`
        SELECT uv.nomination_order_id,
        ...
        FROM user_voting uv
                 JOIN nomination_order no ON uv.nomination_order_id = no.id
                 JOIN users u ON no.user_id = u.id
        WHERE no.nomination_id = :nomination AND uv.type = :type
        GROUP BY uv.nomination_order_id,
                 u.firstname_en,
                 u.firstname_ru,
                 u.lastname_en,
